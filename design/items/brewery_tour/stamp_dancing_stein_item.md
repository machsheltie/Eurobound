# Stamp: The Dancing Stein

- **Item ID**: `stamp_dancing_stein`
- **Brewery**: The Dancing Stein (`sinfonia.dancing_stein`)
- **City**: Sinfonia
- **Earned**: challenge completion at this brewery (never on ordering — author ruling 2026-08-04)

## Design
- **Stamp Face**: traditional beer stein with cartoon legs mid-waltz, foam trailing like a dress [source: design/items/brewery_passport_book_sinfonia.md L152]
- **Ink Color**: ballroom gold with blue waltz accents [source: design/items/brewery_passport_book_sinfonia.md L153]
- **Stamp Animation**: waltz music plays (3 beats), stamp spins onto page in rhythm [source: design/items/brewery_passport_book_sinfonia.md L155]

## The Stamping
- **Stamper**: Herr Drehmeister [source: design/items/brewery_passport_book_sinfonia.md L156]
- **Stamp Speech**: "You waltzed! You carried! You barely spilled! *stamp in rhythm* ONE-two-three, ONE-two-three, STAMPED! Welcome to the dancers, friend!" [source: design/items/brewery_passport_book_sinfonia.md L156]

## PNG Assets
- `stamp_dancing_stein.png` — 64x64 [source: design/items/brewery_passport_book_sinfonia.md L313]
- `stamp_dancing_stein_animation.png` — 64x64x4 [source: design/items/brewery_passport_book_sinfonia.md L317; renamed from the source's shortened `stamp_dancing_animation.png` to the full brewery-stem convention per Contradiction 5 ruling]

## Notes
- No drafted content — every field traces directly to the extraction table (Sinfonia is fully sourced, no gaps).
