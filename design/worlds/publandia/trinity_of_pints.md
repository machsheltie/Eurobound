# Trinity of Pints

*Publandia's most prestigious drinking institution — where "lectures" are drinking contests and every student is perpetually "studying"*

## Location Identity
- **Location ID**: publandia_pub_trinity_pints_01
- **Display Name**: Trinity of Pints
- **Irish Alias**: "Tríonóid na bPiontaí"
- **Satirical Subtitle**: "Cum Laude in Lager"
- **World Location**: Publandia
- **Area Zone**: Adjacent to Cobblestone Market Street (the "academic quarter")
- **Location Type**: Fake University / Themed Pub / Drinking Challenge Venue / Brewery Tour Stop

## Access & Requirements
### Entry Conditions
- **Time Restrictions**: 11 AM - 1 AM ("class hours")
- **Stat Requirements**: None (open enrollment)
- **Item Requirements**: "Tuition" (10 Sovs cover charge) OR proof of previous "graduation"
- **Previous Location Requirements**: None
- **Special Conditions**: Different "courses" available at different times

### Exit Conditions
- **Normal Exit**: Through the ornate gates, diploma optional
- **Completion Requirements**: Complete a "course" to graduate
- **Failure States**: "Academic Probation" status if you fail too many challenges
- **Progressive Unlocks**: "Graduate" status unlocks advanced courses, Alumni Discount

## Visual Design
### Exterior Description
- **Structure**: Georgian building mimicking Trinity College Dublin, but... off
- **Design**: Impressive facade hiding a pub interior, ivy-covered walls (plastic ivy)
- **Gates**: Ornate iron gates with "TRINITY OF PINTS" in faux-Latin script
- **Signage**:
  - Main: "TRINITY OF PINTS — Est. MCMXCVII" (1997, not as old as it looks)
  - Motto: "In Vino Veritas, In Cervisia Felicitas" (In Wine Truth, In Beer Happiness)
  - Enrollment: "NOW ACCEPTING STUDENTS — No Prerequisites Required"
  - Course Board: Daily "lecture" schedule posted
  - Warning (small): "Academic standards may vary"
- **Atmosphere**: Pseudo-academic gravitas undermined by visible drunkenness
- **Environmental Context**: Near actual tourist attractions, catches the "cultural" crowd

### Interior Layout — The Campus

**The Entrance Hall (Registration)**
- Fake portraits of "distinguished alumni" (stock photos in robes)
- "Enrollment desk" where you pay cover and receive student ID
- Course catalog (drinking challenge menu)
- Trophy case of past "achievements" (empty bottles, stained diplomas)

**The Great Hall (Main Pub Area)**
- Long wooden tables mimicking Hogwarts/Oxford dining halls
- "Professors" at head table (bartenders in academic robes)
- Drinking challenge stages scattered throughout
- Banners for various "departments"

**The Library (Back Room)**
- Bookshelves filled with hollow book facades
- Quieter drinking area for "studying" (day drinking with pretense)
- Hidden flask compartments in fake books
- "Research materials" (cocktail recipe books)

**The Lecture Halls (Challenge Rooms)**
- Multiple rooms for different "courses"
- Stadium seating around central challenge area
- Chalkboards with "formulas" (drinking game rules)
- Graduation podium for successful students

**The Quad (Beer Garden)**
- Outdoor seating area
- "Lawn games" (drinking game versions)
- Selfie spot with fake graduation backdrop
- Where "graduation ceremonies" occur

### The Academic Aesthetic
| Element | Reality |
|---------|---------|
| Diplomas on walls | Printed that morning |
| Professor robes | Party store quality |
| Latin mottos | Mostly incorrect |
| Ancient books | Hollow props from IKEA |
| Prestigious portraits | Stock photos |
| Ivy on walls | Plastic, from Amazon |
| Academic bells | Recorded, plays randomly |

### Technical Specifications
- **Color Palette**: Primary: Academic Green (#355E3B), Accent: Scholarly Gold (#CFB53B), Background: Old Parchment (#F5DEB3), Wood Brown (#8B4513)
- **Lighting Setup**: Faux candlelight (LED), stained glass effect, "library" mood
- **Particle Systems**: Dust motes, fake candle flicker, graduation confetti
- **Animation Elements**: Professor lecturing, students drinking, graduation ceremony
- **Performance Optimization**: Simplified Great Hall crowd, static portraits

## Satirical Design
### Theme & Vibe
**Core Concept**: Trinity of Pints is a pub that cosplays as a prestigious university, turning drinking into "education." The joke operates on multiple levels: it satirizes pub tourism gimmicks, academic pretension, the commodification of "the college experience," and most importantly — tourists who want to feel CULTURED while getting hammered.

The bros LOVE it because it makes their drinking feel intellectual. They're not getting drunk; they're "studying." They're not failing at beer pong; they're "struggling with the coursework." The academic framing lets them feel sophisticated while doing the exact same thing they'd do at any bar.

### The Dean Philosophy
Dean Cathal Murphy runs Trinity of Pints with absolute commitment to the bit. He's never broken character in 15 years. Is he a failed academic who found a better path? A performance artist? A genuine believer in "alternative education"? No one knows. He grades your drinking with the seriousness of a tenure committee.

### Cultural Targets
- **Primary Satirical Focus**: Gimmick pubs, "experiential" tourism, drinking dressed up as culture
- **Secondary Targets**: Academic pretension, the "college experience" industry, tourists wanting to seem sophisticated
- **Cultural Specificity**: Trinity College Dublin tourism, Irish literary heritage commodification
- **Humor Approach**: Total commitment to absurd premise, academic language for drinking
- **Sensitivity Considerations**: Satirizes tourism and pretension, not education itself

### Seedy Underbelly Elements
- **Primary Vice**: Charging "tuition" (cover) for what's essentially a themed pub
- **City-Specific Comedy Operations**: Exploiting tourists' desire to feel cultured
- **Buffoon Exploitation**: Bros who desperately want drinking to be intellectual
- **Comedy of Errors**: Failing "academically" at drinking, taking it seriously
- **Consequence System**: "Academic standing" affects treatment, Graduate status unlocks

### Buffoon Delusion Scenarios
**What bros THINK is happening**: Engaging with Irish intellectual tradition, becoming cultured

**What is ACTUALLY happening**: Drinking at a themed bar with a cover charge

**How impaired are they**: Increasingly (it's still a bar)

**The academic delusion**:
- "I'm not drunk, I'm STUDYING"
- "This isn't a drinking game, it's COURSEWORK"
- "I didn't fail, the GRADING CURVE was harsh"
- "My diploma is going on my WALL"
- "I'm basically an ALUMNI of PUBLANDIA now"

**The transformation**:
- Enter seeking culture
- Pay "tuition" (cover charge)
- Take "courses" (drinking challenges)
- Fail "exams" (lose drinking games)
- Graduate anyway (everyone graduates eventually)
- Leave feeling intellectual (actually just drunk)

**How locals interact**: Staff fully commit to bit; actual locals avoid this place entirely

## Gameplay Mechanics
### Enrollment System
**Cover Charge ("Tuition")**: 10 Sovs
- Includes "Student ID" (laminated card with your name)
- Access to all "courses" for the evening
- One free "textbook" (branded pint glass)
- Graduation eligibility

**Graduate Discount**: 5 Sovs (show previous diploma)
- "Alumni" pricing
- Access to "Advanced Courses"
- Reserved seating in Great Hall

### Course Catalog (Drinking Challenges)

**Freshman Courses (Easy)**
| Course | Challenge | Passing Grade | Reward |
|--------|-----------|---------------|--------|
| BEER 101: Introduction to Pints | Finish a pint in under 2 minutes | Complete | +10 Bravado, "Enrolled" status |
| SHOT 110: Fundamentals of Spirits | 3 shots, identify the whiskey | 2/3 correct | +15 Bravado, Whiskey discount |
| PUB 115: Social Drinking Basics | Win a toast-off against NPC | Crowd approval | +10 Bravado, +Craic'd Up |

**Sophomore Courses (Medium)**
| Course | Challenge | Passing Grade | Reward |
|--------|-----------|---------------|--------|
| BEER 201: Comparative Lagers | Blind taste test, 5 beers | 3/5 correct | +20 Bravado, Beer Snob status |
| GAME 215: Applied Darts Theory | Hit specific targets while drinking | 150 points | +15 Bravado, Darts proficiency |
| SONG 220: Traditional Music Studies | Rhythm game, Irish drinking songs | 80% accuracy | +20 Bravado, +Craic'd Up extended |

**Junior Courses (Hard)**
| Course | Challenge | Passing Grade | Reward |
|--------|-----------|---------------|--------|
| BEER 301: Advanced Stout Analysis | Guinness pour + drink + essay (toast) | Dean approval | +30 Bravado, Stout Mastery |
| ENDUR 310: Sustained Consumption | Drinking contest, 5 rounds | Survive Round 4 | +25 Bravado, Iron Liver buff |
| TRIVIA 315: Pub Quiz Championship | Team trivia, 20 questions | 15 correct | +30 Bravado, +party member buffs |

**Senior Thesis (Final Challenge)**
| Course | Challenge | Passing Grade | Reward |
|--------|-----------|---------------|--------|
| THESIS 401: Comprehensive Examination | Multi-stage gauntlet: drink, game, toast, song | Complete all stages | GRADUATION + Diploma + Stamp |

### Graduation Ceremony
**Trigger**: Complete Senior Thesis OR accumulate enough course credits
**Location**: The Quad (weather permitting) or Great Hall
**Ceremony**:
1. Dean Cathal calls your name
2. Walk to podium (balance check based on intoxication)
3. Receive diploma (actual printed certificate)
4. "Commencement toast" (final drink)
5. Photo opportunity at graduation backdrop
6. Brewery Passport stamp applied

**Diploma Text**:
```
TRINITY OF PINTS
This certifies that
[PLAYER NAME]
has completed the rigorous curriculum of
THE SCHOOL OF LIQUID ARTS
and is hereby granted the degree of
BACHELOR OF BEVERAGE SCIENCES (B.B.S.)
"In Cervisia Felicitas"
[Dean Cathal Murphy signature]
```

### Academic Standing System
| GPA | Status | Effect |
|-----|--------|--------|
| 3.5-4.0 | Dean's List | 20% discount, VIP seating, staff respect |
| 2.5-3.4 | Good Standing | Normal treatment |
| 1.5-2.4 | Academic Probation | Mocking from staff, "remedial" courses required |
| Below 1.5 | Expulsion Risk | Must retake courses, public shame |

**GPA Calculation**:
- A (Pass with honors): 4.0
- B (Pass): 3.0
- C (Barely pass): 2.0
- F (Fail): 0.0

### Bravado Interactions
- **Gain Bravado**: Pass courses, graduate, give good toasts, take academic roleplay seriously
- **Lose Bravado**: Fail courses (especially easy ones), break character, question the premise
- **Special**: "Valedictorian" (highest GPA of the night) gets bonus Bravado + free drinks

### Status Effects (Location-Specific)
- **"Enrolled"**: Access to courses, part of the bit
- **"Scholar's Focus"**: +INT temporarily after certain courses
- **"Academic Probation"**: Must pass remedial course to restore standing
- **"Graduate"**: Permanent discount, respect, Advanced Course access
- **"Valedictorian"**: Temporary massive Bravado boost, staff fawn over you

## NPCs & Staff
### The Administration

**Dean Cathal Murphy**
- **Role**: Owner/operator, head "professor," never breaks character
- **Appearance**: 50s Irish man, full academic regalia (robe, cap, medallion), perpetually serious
- **Background**: Former actual lecturer (adjunct, let go), found calling in "alternative education"
- **Demeanor**: Absolute commitment to the bit, grades drinking with scholarly gravitas
- **The Cathal Method**: Treat every pint like a thesis defense
- **Philosophy**: "Education takes many forms. Who's to say this isn't one of them? The Socratic method involved a lot of wine, you know."

**Dean Cathal's Dialogue Categories**:

*The Welcome:*
- "Welcome to Trinity of Pints. I am Dean Murphy. Your education begins now."
- "Enrollment is 10 Sovs. This covers tuition, materials, and access to our distinguished faculty."
- "I see potential in you. Let's see if you can live up to it. Academically speaking."

*During Courses:*
- "Your form is acceptable. Your commitment, however, needs work."
- "A gentleman's C. You've passed, but you haven't excelled. Consider extra credit."
- "Disappointing. I expected more from someone of your... enthusiasm. Remedial coursework is available."
- "Outstanding! You've demonstrated mastery of the material. The faculty is impressed."

*Graduation:*
- "Today, we gather to honor those who have completed the rigorous curriculum of Trinity of Pints."
- "Through dedication, perseverance, and a truly impressive tolerance, you have earned this honor."
- "I hereby confer upon you the degree of Bachelor of Beverage Sciences. Go forth and drink responsibly. Or don't. You've earned it."

*Private (Rare, If Pushed):*
- "Is this real? Is anything real? I was an adjunct for twelve years. I taught Romantic poetry to students who couldn't name a single Romantic poet. THIS is more honest."
- "They learn something here. Camaraderie. Humility. How to lose gracefully. That's more than most universities teach."

### Faculty

**Professor Róisín O'Sullivan (Spirits Department)**
- **Role**: Whiskey courses, shot challenges, drinks history
- **Appearance**: 40s Irish woman, tweed jacket, reading glasses, perpetually holding a glass
- **Demeanor**: Warm but exacting, disappointed when you can't identify Jameson vs. Powers
- **Dialogue**:
  - "Whiskey is a language. Today, you learn to speak it. Or at least slur it correctly."
  - "Close your eyes. Smell first. Now taste. What do you detect? ...No, 'burning' is not a tasting note."
  - "A passing grade. You've learned the basics. The advanced course involves blind identification while reciting Yeats."

**Professor Cian MacCarthy (Games & Recreation)**
- **Role**: Darts, pool, drinking game challenges
- **Appearance**: 30s Irish man, sports casual under academic robe, competitive energy
- **Demeanor**: Encouraging but will absolutely destroy you at darts
- **Dialogue**:
  - "Darts isn't just a game. It's physics, psychology, and BAC management in perfect harmony."
  - "You're overthinking it. Also underthinking it. Find the middle path. While drunk."
  - "Not bad! You've got potential. Of course, I was state champion in '08, so my standards are high."

**Professor Aisling Byrne (Traditional Music)**
- **Role**: Drinking song challenges, rhythm games, toast poetry
- **Appearance**: 20s Irish woman, folk musician aesthetic, always has a bodhrán nearby
- **Demeanor**: Enthusiastic, forgiving of bad singing, ruthless about rhythm
- **Dialogue**:
  - "The Irish drinking song is a proud tradition. You will learn it. You will love it. You will remember approximately 40% of the lyrics."
  - "Feel the rhythm! The beat is in your heart! ...or that's the palpitations from the caffeine mixer."
  - "Beautiful! Terrible, but beautiful! That's the Irish way!"

### Student NPCs

**The Repeat Student**
- **Composition**: Someone who keeps coming back, never quite graduates
- **Behavior**: Taking BEER 101 for the fifth time
- **Dialogue**: "I keep failing the thesis. The Dean says I have 'commitment issues.' He's not wrong."
- **Function**: Shows the long-term "student"

**The Overachiever**
- **Composition**: Tourist taking this WAY too seriously
- **Behavior**: Taking notes, asking about "extra credit"
- **Dialogue**: "I've completed every course. I'm going for a DOUBLE MAJOR. Beer AND Spirits."
- **Function**: Shows the over-committed tourist

**The Legacy Admission**
- **Composition**: Someone whose parents "graduated" here
- **Behavior**: Expecting preferential treatment
- **Dialogue**: "My dad graduated in '04. Class valedictorian. I expect the same standards to apply."
- **Dean's Response**: "Legacy status grants you nothing here. Only the pint matters."
- **Function**: Satirizes legacy admissions

**The Study Group**
- **Composition**: Bachelor/bachelorette party treating this as activity
- **Behavior**: Matching shirts, coordinated cheering, competitive
- **Dialogue**: "Sarah's getting MARRIED so she BETTER graduate!"
- **Function**: Group dynamic comedy

### Original NPC Dialogue
**Dean Cathal's Philosophy (Extended):**
- "I spent twelve years in academia. Publish or perish, they said. I perished. Then I found THIS. Now I publish diplomas. Who's the failure now?"
- "Every great university has traditions. Ours is drinking. Oxford has rowing. We have pints. I fail to see the difference."
- "The students who come here... they want something. Connection. Achievement. An excuse to drink with purpose. I provide all three."
- "My doctoral thesis was on the semiotics of Irish identity in post-colonial literature. Now I grade pint-finishing technique. I've never been happier."
- "The diploma is fake. The education is real. Think about it."

**The Bros' Trinity of Pints Reactions:**
- **Lord Pilsner** (entering): "A UNIVERSITY for DRINKING? This is the greatest country on EARTH."
- **Lord Pilsner** (failing): "The curve must be HARSH. This is GRADUATE-LEVEL drinking."
- **Lord Pilsner** (graduating): "I'm putting this on my LINKEDIN. Bachelor of Beverage Sciences. HR will be IMPRESSED."
- **Chadwick**: "Academic content is TRENDING. This is EDUCATIONAL ENTERTAINMENT."
- **Chadwick** (filming): "And here we see the RIGOROUS STANDARDS of Irish higher education..."
- **Chadwick** (graduating): "I have a DEGREE in CONTENT now. That's basically what this is."
- **Bradley**: "Is the accreditation... current?"
- **Dean Cathal**: "You may audit SKEPTIC 101: Questioning Everything."
- **Bradley** (graduating): "I'm... proud? I didn't expect to feel genuine emotion at a fake graduation."

## Brewery Tour Integration

### Stamp Acquisition
**Requirement**: Must have Bottle Opener Keychain from The Blarney Trap
**Unlock Condition**: GRADUATE from Trinity of Pints (complete Senior Thesis)
**Stamp Location**: Applied during graduation ceremony by Dean Cathal

**Stamp Design**:
- Square academic seal shape
- Open book with pint glass in center
- Laurel wreath border
- Banner text: "Trinity of Pints"
- Bottom text: "Cum Laude in Lager"
- Ink color: Deep navy blue

**Dean Cathal's Stamp Dialogue**:
- "Ah, you carry Seamus's opener. A fellow traveler on the Continental Trail."
- "Your passport, please. This stamp represents genuine academic achievement. In drinking."
- "There. You are now officially an alumnus of Trinity of Pints. The beer world recognizes your accomplishment."

### Signature Beer: "Academic Amber"
**Brewed By**: The Stoutworks Brewery (contract brew for Trinity of Pints)
**Style**: Irish Red Ale with honey notes
**ABV**: 4.8%
**Unlocked**: When stamp is acquired (graduation)
**Availability**: Trinity of Pints, The Blarney Trap (after unlock)

**"Academic Amber" Stats**:
| Stat | Value |
|------|-------|
| HP Restoration | +25 HP |
| SP Restoration | +20 SP |
| Special Effect | "Scholar's Focus" — +3 INT for 30 minutes |
| Status Risk | 10% chance of "Pretentious" (dialogue becomes insufferable) |
| Purchase Price | 9 Sovs |

**Flavor Text**: "Brewed for minds that think they're smarter when drinking. They're not. But the confidence helps. Tastes like unearned intellectual superiority."

**Dean Cathal on Academic Amber**:
- "Our signature brew. Stoutworks makes it to our specifications. Light enough to drink during lectures, complex enough to pretend you're sophisticated."
- "The honey notes represent the sweetness of knowledge. The alcohol represents everything else."
- "Students report improved cognitive function. They're wrong, but they FEEL smarter. Isn't that what education is for?"

### Brewmaster Selfie Spot
**Location**: Graduation podium in The Quad
**Backdrop**: Fake academic banners, ivy-covered wall, Trinity of Pints crest
**Props Available**: Academic robe, mortarboard cap, diploma
**Special Variant**: "Valedictorian" photo (if achieved highest GPA of session)

**Selfie Requirements**:
- Must have graduated
- Optional: Wear provided academic regalia
- Optional: Hold diploma in frame

---

## System Integration
### Academic Hub Network
- **Function**: Unique challenge venue, Brewery Tour stamp location
- **Connection to Blarney Trap**: Seamus recommends it; Academic Amber available there post-unlock
- **Connection to Stoutworks**: They brew Academic Amber; cross-promotion
- **Theme**: Education as drinking excuse, culture tourism gimmicks

### Cross-Location Dependencies
- **The Blarney Trap**: Quest mention, beer availability after graduation
- **The Stoutworks Brewery**: Brews Academic Amber, academic NPCs reference it
- **Cobblestone Market Street**: Visible from main street, draws "cultural" tourists

### Achievement Track
- **Enrolled**: "Freshman" — paid tuition, received student ID
- **First Course Passed**: "Sophomore" — one credit earned
- **Three Courses Passed**: "Junior" — halfway there
- **Graduated**: "Bachelor" — diploma earned, stamp acquired
- **Valedictorian**: "Summa Cum Laude" — highest GPA achievement
- **All Courses Completed**: "Scholar" — exhaustive curriculum mastery
- **Graduate Status Used**: "Alumni" — returned with discount

---

## Audio Design
### Environmental Audio
- **Ambient Soundscape**:
  - "Academic" bell tolling (marks course times)
  - Scholarly murmuring (actually drinking chatter)
  - Professor lecturing (drinking instructions)
  - Cork popping (graduation celebrations)
  - Traditional music (background, for courses)
- **Music Style**: Faux-classical with Irish pub fusion
- **Audio Transitions**: More celebratory during graduations
- **Interactive Audio**: Course completion fanfare, graduation ceremony music

### Comedy Audio Integration
- **Satirical Stingers**: Graduation "Pomp and Circumstance" parody
- **Character Voice Direction**: Dean's scholarly gravitas, professors' academic tones
- **Environmental Comedy**: Drunk students attempting "academic" discussions
- **The Diploma Sound**: Official stamp + cork pop combination

---

## Implementation Specifications
### Technical Requirements
- **Asset List**:
  - Georgian building exterior
  - Great Hall interior
  - Library room
  - Lecture halls (multiple)
  - The Quad beer garden
  - Graduation podium
  - Dean Cathal and professors
  - Student NPCs
  - Academic decor (fake books, portraits, diplomas)
  - Course challenge setups
- **Animation Requirements**:
  - Course challenges (drinking, darts, rhythm)
  - Graduation ceremony
  - Dean's lecturing gestures
  - Diploma presentation
- **Performance Budget**: 60 FPS target, max 16 draw calls, 38MB memory
- **Platform Considerations**: Simplified Great Hall crowd, static portraits

### Development Timeline
- **Asset Creation**: 10 hours (academic aesthetic, multiple rooms, many NPCs)
- **Implementation Phase**: 12 hours (course system, GPA tracking, graduation ceremony)
- **Testing Requirements**: Course difficulty balance, GPA calculation, ceremony triggers
- **Polish Phase**: 5 hours (academic atmosphere, Dean's delivery, ceremony pacing)

---

## PNG Asset Requirements Summary

### Environment Art (12 PNGs)
- trinity_exterior.png - 384x384 - Georgian facade with gates
- great_hall.png - 512x384 - Main pub area
- library_room.png - 384x256 - Fake book aesthetic
- lecture_hall.png - 384x256 - Challenge room
- the_quad.png - 384x384 - Beer garden graduation space
- graduation_podium.png - 192x256 - Ceremony focal point
- enrollment_desk.png - 256x192 - Registration area
- trophy_case.png - 192x256 - Empty bottles and stained diplomas
- academic_banners.png - 128x256 - Department flags
- fake_portraits.png - 256x192 - Distinguished "alumni"
- bookshelf_hollow.png - 256x256 - The Library deception
- academic_decor_set.png - 384x192 - Robes, caps, props

### Character Sprites (10 PNGs)
- dean_declan.png - 64x96 - Full academic regalia
- dean_lecturing.png - 64x96x3 - Teaching animations
- dean_graduating.png - 64x96x2 - Ceremony poses
- prof_siobhan.png - 64x96 - Spirits department
- prof_cian.png - 64x96 - Games department
- prof_aisling.png - 64x96 - Music department
- repeat_student.png - 64x96 - Fifth time BEER 101
- overachiever.png - 64x96 - Taking notes
- study_group.png - 192x96 - Bachelor party coordination

### Item/UI Assets (14 PNGs)
- student_id.png - 64x48 - Enrollment card
- diploma_template.png - 192x128 - Graduation certificate
- course_catalog_ui.png - 256x384 - Challenge menu
- gpa_tracker.png - 128x64 - Academic standing
- stamp_trinity_pints.png - 64x64 - Brewery passport stamp
- academic_amber_bottle.png - 32x64 - Signature beer
- academic_amber_icon.png - 32x32 - Inventory icon
- course_complete_badge.png - 48x48 - Individual course
- graduation_cap_icon.png - 32x32 - Graduate status
- valedictorian_medal.png - 48x48 - Top achievement
- scholars_focus_buff.png - 32x32 - INT boost icon
- pretentious_debuff.png - 32x32 - Status effect
- selfie_frame_academic.png - 192x256 - Photo backdrop
- academic_robe_equip.png - 64x96 - Wearable prop

---

*Trinity of Pints: Where drinking becomes "education," failure becomes "academic probation," and your diploma is worth exactly what you paid for it — plus the memory of Dean Cathal calling your name with genuine scholarly pride. Cum Laude in Lager, friends. Cum Laude in Lager.*
