...
Desktop UX Optimization – keyboard/mouse + controller input, UI anchors, responsive resolution scaling
Version Control (Git) – commit early/often, branch for features
Desktop Build Process – export from Godot with desktop export templates, run on target hardware

Soft Skills (Creative & Project Management):
Timeboxing – setting small, achievable build goals each week
Iterative Testing – playing real desktop builds early and often to catch UX issues
...
Test keyboard/controller feel in the desktop export early, before adding more content
...
OS Support Matrix – minimum Windows/macOS/Linux versions, graceful degrade for older hardware.
...
--- NEW SECTION ---
Phaser.js Performance & Loading:
- Performance profiling on min-spec target hardware as baseline; ensure battle scenes run <50ms frame times.
...
- Label bugs with tags like “Platform-Specific,” “Satire-Text,” “Combat Logic” for quick filtering.
...
- Automated flow: Build → Steam beta branch/Itch.io private release → QA sign-off → push live.
...
- Recruit persona-based testers: satire enjoyers, RPG min-maxers, casual dabblers.
...
- In-region reviewer ensures cultural accuracy; final decision for inclusion rests with developer (you) and Valve for store approval.
...
- One Unified Pre-Merge Checklist for all pipelines (art, writing, code, audio):
  - Performance — FPS/memory checks, asset size budgets, input responsiveness.
...
- All checks performed in final runtime context (UI in the desktop export, audio in-engine, code in latest build).

### Performance Profiling Targets
- Baseline: min-spec target hardware.
- Pass criteria:
