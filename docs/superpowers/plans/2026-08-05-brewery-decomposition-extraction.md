...
| 9 | **Systemic beer-item PNG gap.** Every signature beer in the extraction table has a cited bottle+icon PNG pair (e.g. Sweet Surrender: `sweet_surrender_bottle.png` 32x64 + `sweet_surrender_icon.png` 32x32), and the master spec's own "What Every Brewery Needs" checklist requires "PNG asset list: stamp sheet, beer bottle + icon, brewmaster/selfie assets" per brewery | **RESOLVED 2026-08-05 (fix pass) for 11 of 12.** HOME: all 11 remaining `beer_*_item.md` files now carry a `## PNG Assets` section (full-brewery-stem filenames per Contradiction 5's ruling extension to beer assets, e.g. `beer_blarney_trap_bottle.png`/`beer_blarney_trap_icon.png`, replacing the sources' beer-name stems like `sweet_surrender_bottle.png`), each citing its extraction-table source line — verified open. `beer_shenanigans_item.md` (the 12th) already had PNGs from the original Gap #20 resolution, using a beer-name stem (`fools_gold_bottle.png`) rather than the brewery-stem now applied elsewhere — flagged as a residual naming inconsistency for author awareness, not re-touched here (outside this fix pass's authorized scope). Root cause fixed at the source: `design/templates/signature_beer_item_template.md` now carries a `## PNG Assets` heading matching the stamp/opener templates, with a dated note flagging the omission for author awareness |
...
Progression table into `brewerytoursinfonia.md`. Two non-blocking concerns remain on the
record (not re-litigated here): the Master's Toast section isn't duplicated into
`brewerytourdebaucheryville.md` (A.3 #5), and `beer_shenanigans_item.md`'s existing PNG names
use a beer-name stem rather than the brewery-stem convention now applied to the other 11 beers
(A.4 #9). Retirement (banner-prepending, `git mv` to `design/legacy/`, Source Reconciliation
flip, NEXT_WORK completion edit) proceeds below.
