...
| 3 | Full Brewery Tour Summary — Difficulty Progression table (Sinfonia = Performance/Social, Medium, "Retry with purchase, social penalties") | **RESOLVED 2026-08-05 (fix pass).** HOME: `design/quests/sinfonia/brewerytoursinfonia.md` § Brewery Tour Difficulty Progression (cross-city) — all 4 rows transcribed verbatim [source: design/items/brewery_passport_book_sinfonia.md L369-374], with an ⚠ AUTHOR-REVIEW note flagging the unreconciled "retry with purchase" language against the earned-opener ruling — verified open |
...
| 6 | Sinfonia stamp/beer/page PNG specs (four 64x64x4 animation sheets; `passport_page_sinfonia.png` 256x384; `legendary_hangover_sinfonia.png`) | HOME (animation sheets + page PNG): all 4 `stamp_*_item.md` (Sinfonia) § PNG Assets, `brewery_passport_book_item.md` § Book Assets — verified open. `legendary_hangover_sinfonia.png`: **RESOLVED 2026-08-05 (fix pass).** HOME: `brewery_passport_book_item.md` § Achievement Assets [source: design/items/brewery_passport_book.md L365-371] — verified open |
...
| 6 | ALL Debaucheryville stamp/beer/page PNG specs (8 stamp incl. 4× 64x64x4 animation, 8 beer 32x64+32x32, `passport_page_debaucheryville.png`, `legendary_hangover_debaucheryville.png` 48x48) | HOME (stamps, 8 PNGs across 4 files; page PNG): all 4 `stamp_*_item.md` (Deb) § PNG Assets, `brewery_passport_book_item.md` § Book Assets — verified open. Beer PNGs (8): **RESOLVED 2026-08-05 (fix pass).** HOME: `beer_absinthium_taproom_item.md`, `beer_tap_dat_vault_item.md`, `beer_three_crown_keg_haus_item.md`, `beer_ye_olde_brew_lab_item.md` § PNG Assets (new sections, full-brewery-stem filenames per Contradiction 5, sourced from `design/items/brewery_passport_book_debaucheryville.md` L283-290) — verified open. `legendary_hangover_debaucheryville.png`: **RESOLVED 2026-08-05 (fix pass).** HOME: `brewery_passport_book_item.md` § Achievement Assets — verified open |
...
| 7 | ~40 PNG specs — Book Assets, generic Stamp Assets (`stamp_slot_empty.png`, `stamp_slot_locked.png`), Publandia beer bottles/icons, Achievement Assets, all 8 UI Assets | Book Assets: HOME, `brewery_passport_book_item.md` § Book Assets — verified open. UI Assets (8): HOME, same file § UI Assets — verified open, count matches. Generic Stamp Assets (`stamp_slot_empty.png`/`stamp_slot_locked.png`): **RESOLVED 2026-08-05 (fix pass).** HOME: `brewery_passport_book_item.md` § Stamp Assets (Generic), new subsection [source: design/items/brewery_passport_book.md L351-352] — verified open. Publandia beer bottles/icons: **RESOLVED 2026-08-05 (fix pass).** HOME: `beer_blarney_trap_item.md`, `beer_stoutworks_brewery_item.md`, `beer_trinity_of_pints_item.md` § PNG Assets, new sections (full-brewery-stem filenames per Contradiction 5, sourced from `design/items/brewery_passport_book.md` L357-362); `beer_shenanigans_item.md` already had its own (added ad hoc while resolving Gap #20) — verified open, all 4 of 4 Publandia beers now covered. Achievement Assets: see #8 |
| 8 | Achievement Assets (6 PNGs): `legendary_hangover_publandia.png`, `_debaucheryville.png`, `_sinfonia.png`, `_shamsterdam.png` (all 48x48), `continental_beer_master_badge.png` (96x96), `photogenic_drinker_badge.png` (64x64) | **RESOLVED 2026-08-05 (fix pass) for 5 of 6.** HOME: `brewery_passport_book_item.md` § Achievement Assets, new subsection [source: design/items/brewery_passport_book.md L365-371] — verified open, all 5 in-scope entries present verbatim (dimensions matched). Shamsterdam's icon: DEFERRED (1 of 6, out-of-scope reservation noted alongside) |
| 9 | **Systemic beer-item PNG gap.** Every signature beer in the extraction table has a cited bottle+icon PNG pair (e.g. Sweet Surrender: `sweet_surrender_bottle.png` 32x64 + `sweet_surrender_icon.png` 32x32), and the master spec's own "What Every Brewery Needs" checklist requires "PNG asset list: stamp sheet, beer bottle + icon, brewmaster/selfie assets" per brewery | **RESOLVED 2026-08-05 (fix pass) for 12 of 12.** HOME: all 12 `beer_*_item.md` files now carry a `## PNG Assets` section using full-brewery-stem filenames per Contradiction 5's ruling extension to beer assets (e.g. `beer_blarney_trap_bottle.png`/`beer_blarney_trap_icon.png`, replacing the sources' beer-name stems like `sweet_surrender_bottle.png`), each citing its extraction-table source line — verified open. `beer_shenanigans_item.md` (the 12th) was corrected in a follow-up fix pass (2026-08-05, same day): its original ad hoc Gap #20 resolution had used a beer-name stem (`fools_gold_bottle.png`/`fools_gold_icon.png`); it now reads `beer_shenanigans_bottle.png`/`beer_shenanigans_icon.png`, inline-noted as "[renamed from fools_gold_* per the full-brewery-stem ruling]" — verified open, no residual inconsistency remains. Root cause fixed at the source: `design/templates/signature_beer_item_template.md` now carries a `## PNG Assets` heading matching the stamp/opener templates, with a dated note flagging the omission for author awareness |
| 10 | Implementation Notes section (Technical Requirements, Save Data, UI/UX Priorities) | **RESOLVED 2026-08-05 (fix pass).** HOME: `brewery_passport_book_item.md` § Implementation Notes, new section transcribed verbatim [source: design/items/brewery_passport_book.md L310-330] — verified open |
...
- **HOME**: 52 rows (fully resolved to an open, verified file/section — 44 original + 8 resolved in the 2026-08-05 fix pass)
...
- **BLOCKED — no home, no kill ruling**: **0 rows.** All 8 clusters resolved 2026-08-05 (fix pass) — see A.2 #3, A.2 #6, A.3 #6, A.4 #7, A.4 #8, A.4 #9, A.4 #10 above.

**Verdict (updated 2026-08-05, fix pass): ALL ROWS RESOLVE.** The 8 BLOCKED clusters were
content-completion gaps in Tasks 5–7's item-file writing, not open author questions — every
underlying fact was already sourced and cited in the Extraction Table above. Fixed by: adding
a `## PNG Assets` heading to `design/templates/signature_beer_item_template.md` (root cause;
the stamp/opener templates already had one) and backfilling it into all 11 affected
`beer_*_item.md` files verbatim from the Extraction Table's citations, using full-brewery-stem
filenames per Contradiction 5's ruling; adding the dropped Achievement Assets bucket (5 of 6
in-scope PNGs) and a generic Stamp Assets subsection to `brewery_passport_book_item.md`;
restoring its Implementation Notes section verbatim; and transcribing the Sinfonia Difficulty
Progression table into `brewerytoursinfonia.md`. **Follow-up fix (2026-08-05, same day):**
`beer_shenanigans_item.md`'s PNG names were also corrected from the beer-name stem
(`fools_gold_bottle.png`/`fools_gold_icon.png`) to the full-brewery-stem convention
(`beer_shenanigans_bottle.png`/`beer_shenanigans_icon.png`, inline-noted as renamed), so all
12 of 12 beer files are now consistent — no residual naming inconsistency remains. One
non-blocking concern remains on the record (not re-litigated here): the Master's Toast
section isn't duplicated into `brewerytourdebaucheryville.md` (A.3 #5). Retirement
(banner-prepending, `git mv` to `design/legacy/`, Source Reconciliation flip, NEXT_WORK
completion edit) proceeds below.
