# The Brewery Passport Book — UPDATED

*Your leather-bound ticket to the Continental Beer Trail — collect stamps, unlock beers, become legend*

## System Overview

### What Is It?
The Brewery Passport Book is a collectible menu item that tracks the player's progress through the Brewery Tour sidequest. It functions as both a physical in-game item and a dedicated menu interface, displaying stamps collected from breweries and pubs across all cities in the game.

### Acquisition
- **Location**: The Blarney Trap, Publandia
- **Method**: Given FREE with purchase of the Bottle Opener Keychain (15 Sovs) OR as reward for winning a Pint-Off challenge
- **Requirement**: Cannot participate in Brewery Tour without both items
- **Timing**: Available from first visit to The Blarney Trap

### Seamus's Introduction
*"This book? It's your passport to the real beer culture of Europe. Every brewery, every pub worth its salt — they'll stamp you. Proves you were there. REALLY there. Fill a city's page, you earn their Legendary Hangover. Fill the whole book? Continental Beer Master. That's not just a title. That's a LIFESTYLE."*

---

## City-Specific Bottle Openers

Each city has its own Bottle Opener Keychain design that must be acquired to access that city's brewery tour locations.

### Publandia Bottle Opener
- **Design**: Celtic knot shamrock with bottle opener integrated into stem
- **Acquired**: The Blarney Trap (15 Sovs or Pint-Off reward)
- **Material**: "Authentic" brass (spray-painted zinc)
- **Inscription**: "May Your Glass Never Be Empty"

### Debaucheryville Bottle Opener
- **Design**: Neon green absinthe fairy silhouette with bottle opener wings
- **Acquired**: Shadow Exchange (20 Sovs or trade for Publandia stamp proof)
- **Material**: Chrome with glow-in-dark green accents
- **Inscription**: "Reality Is Optional After Dark"
- **Special Feature**: Glows faintly in darkness (UV reactive paint)

### Sinfonia Bottle Opener
- **Design**: Musical note (eighth note) with bottle opener as the flag
- **Acquired**: TBD location
- **Material**: TBD
- **Inscription**: TBD

### Shamsterdam Bottle Opener (Expansion)
- **Design**: TBD
- **Acquired**: TBD
- **Material**: TBD
- **Inscription**: TBD

---

## City Pages

### Publandia (4 Stamps) — COMPLETE
| Slot | Location | Stamp Design | Signature Beer | Unlock Condition |
|------|----------|--------------|----------------|------------------|
| 1 | The Blarney Trap | Celtic harp, emerald ink | The Sweet Surrender | Pint-Off/Toast/Regular status |
| 2 | The Stoutworks Brewery | Copper still, bronze ink | Vat Master's Reserve | Complete tour or dungeon |
| 3 | Trinity of Pints | Scholarly cap, navy ink | Academic Amber | Graduate (complete Senior Thesis) |
| 4 | Shenanigans | Wilted clover, faded gold ink | The Fool's Gold | Lose 3 scams AND return |

**Page Theme**: Emerald green background, Celtic knot borders
**Legendary Hangover**: "The Full Irish Experience" (+15 all stats for battle, then Legendary Hungover -10 all stats 30 min)

---

### Debaucheryville (4 Stamps) — COMPLETE
| Slot | Location | Stamp Design | Signature Beer | Unlock Condition |
|------|----------|--------------|----------------|------------------|
| 1 | The Absinthium Taproom | Green fairy in cracked glass, emerald ink | Absinthe Lager | Complete mirror puzzle + defeat Green Fairy Wisp |
| 2 | The Tap Dat Vault | Neon soundwave in vault door, cyan ink | Vape Stout | Complete rhythm puzzle + defeat Strobe Brewmistress |
| 3 | Three-Crown Keg Haus | Three rusted crowns over stein, rust-orange ink | Iron Gut Tripel | Complete stealth puzzle + defeat Keg Sentinel |
| 4 | Ye Olde B.R.E.W. Lab | Pixelated hop cone with blockchain hash, neon blue ink | Blockchain Bock | Complete taste profile puzzle + defeat BrewBot |

**Page Theme**: Neon purple background, absinthe bottle borders with circuitry accents
**Legendary Hangover**: "The Neon Nightmare" (unique effects below)

#### Debaucheryville Stamp Details

**Stamp 1: The Absinthium Taproom**
- **Stamp Visual**: Green fairy silhouette trapped inside cracked absinthe glass
- **Ink Color**: Shifting emerald (appears to move in certain light)
- **Inscription**: "I Saw Myself"
- **Stamp Animation**: Green mist swirls before stamp lands with ethereal chime
- **Dr. Vaněk's Stamp Speech**: "Congratulations. You've seen yourself. The stamp is proof. The memory is punishment."

**Stamp 2: The Tap Dat Vault**
- **Stamp Visual**: Soundwave pattern emerging from vault door shape
- **Ink Color**: Electric cyan with hot pink accents
- **Inscription**: "Inhaled & Survived"
- **Stamp Animation**: Bass drop sound effect, stamp pulses with rhythm before landing
- **Strobe Brewmistress's Stamp Speech**: "You... kept... the beat... *glitch* ...drink responsibly... *static* ...or don't..."

**Stamp 3: Three-Crown Keg Haus**
- **Stamp Visual**: Three rusting crowns arranged above a foaming stein
- **Ink Color**: Rust orange with faded royal purple accents
- **Inscription**: "Noble by Liver"
- **Stamp Animation**: Regal fanfare (off-key), stamp lands with sticky thud
- **Bartender's Stamp Speech**: "You've joined the nobility! We invented beer before hygiene — and look at you, still standing! Mostly!"

**Stamp 4: Ye Olde B.R.E.W. Lab**
- **Stamp Visual**: Pixelated hop cone with scrolling blockchain hash numbers
- **Ink Color**: Neon blue with green circuit traces
- **Inscription**: "Algorithm Verified"
- **Stamp Animation**: Digital processing sounds, stamp "downloads" onto page
- **BrewBot's Stamp Speech** (if defeated): "Achievement... unlocked... brewery tour... OPTIMIZED... *sparking*"
- **Robotic Bartender's Stamp Speech** (if BrewBot bypassed): "Congratulations! Your taste profile has been... SYNERGIZED! Welcome to the Brewvolution™!"

---

### Debaucheryville Signature Beers (Detailed)

**Absinthe Lager (The Absinthium Taproom)**
- **Style**: Absinthe-infused lager (legally questionable)
- **ABV**: 8.5% (plus "additives")
- **Stats**: +15 SP restoration
- **Effect**: 30% chance to apply "Green Fairy Confusion" to USER
- **Risk**: The confusion is part of the experience
- **Price**: 12 Sovs
- **Flavor Text**: "It's not a bug, it's a feature. The disorientation means it's working." — Dr. Vaněk
- **Brewed By**: The Absinthium Taproom (in-house)
- **Special**: Only beer that can debuff the user by design

**Vape Stout (The Tap Dat Vault)**
- **Style**: Vapor-infused experimental stout
- **ABV**: 6.9% (nice)
- **Stats**: +20 SP restoration
- **Effect**: "Glow-Up Aura" (+1 Charm, +1 Defense for 30 min)
- **Risk**: Applies "Grease Fingers" status (reduced item accuracy)
- **Price**: 14 Sovs
- **Flavor Text**: "We don't serve beer — we inhale it. Your lungs are just another glass."
- **Brewed By**: The Tap Dat Vault (vapor-extraction process)
- **Special**: Consumed as vapor cloud animation

**Iron Gut Tripel (Three-Crown Keg Haus)**
- **Style**: Slavic-style triple-fermented strong ale
- **ABV**: 11.5%
- **Stats**: +40 HP restoration
- **Effect**: "Inspired" status (+10 Bravado for 30 min)
- **Risk**: -10% Accuracy next turn (the strength hits hard)
- **Price**: 10 Sovs
- **Flavor Text**: "300 years of brewing tradition. 0 years of health code compliance. Priorities."
- **Brewed By**: Three-Crown Keg Haus (ancient recipe, questionable equipment)
- **Special**: Served in antique goblet (return for 2 Sovs deposit)

**Blockchain Bock (Ye Olde B.R.E.W. Lab)**
- **Style**: "AI-optimized" dark bock
- **ABV**: 7.2%
- **Stats**: +20 HP, +10 Sovs (crypto fluctuation bonus)
- **Effect**: Applies "Tourist Trapped" status (increased vulnerability to scams)
- **Risk**: The Sovs bonus may be negative if "market crashes"
- **Price**: 18 Sovs (fluctuates ±5 Sovs based on fake crypto ticker)
- **Flavor Text**: "Brewed by algorithms. Verified by blockchain. Tasted by... actually, the AI doesn't taste. That's YOUR job."
- **Brewed By**: Ye Olde B.R.E.W. Lab (random number generator)
- **Special**: Price literally changes each time you check the menu

---

### Debaucheryville Legendary Hangover

**"The Neon Nightmare"**
- **Unlock**: All 4 Debaucheryville stamps collected
- **Effect**: +20 to ALL stats for entire battle, +Confusion immunity during battle
- **Consequence**: "Neon Nightmare Hungover" status after battle
- **Neon Nightmare Hungover Effects**:
  - -15 all stats for 45 minutes
  - Random control reversal (10% chance per input)
  - Screen occasionally flickers with "glitch" effect
  - NPCs comment on your appearance
- **Cure**: Full meal (Plot Armor Provisions equivalent), sleep, or 45 min wait
- **Price**: 75 Sovs (repurchasable from any stamped Debaucheryville location)
- **Flavor Text**: "Four stamps. Four different ways to question reality. The night was legendary. The morning is punishment."

**Strategic Use**:
- Stronger than Publandia's Legendary Hangover (+20 vs +15)
- Longer recovery period (45 min vs 30 min)
- Additional "glitch" effects make post-battle navigation harder
- Best saved for major boss fights in Debaucheryville

---

### Debaucheryville Selfie Spots

| Location | Selfie Spot | Standard Photo | Special Variant |
|----------|-------------|----------------|-----------------|
| The Absinthium Taproom | Floating absinthe chalice behind bar | With glowing green mist | With Dr. Vaněk (complete his "research survey") |
| The Tap Dat Vault | "You Are Brewtiful" hologram wall | Neon glow, vapor clouds | With Strobe Brewmistress silhouette (post-boss) |
| Three-Crown Keg Haus | Velvet keg throne | "Drunken monarchy" pose | With Keg Sentinel wreckage (post-boss) |
| Ye Olde B.R.E.W. Lab | Algorithm Archive terminal | Blockchain hash background | With defeated BrewBot (post-boss) |

**Full Debaucheryville Selfie Collection Reward**:
- **"Neon Photogenic"** badge
- +5% resistance to "Confused" status permanently
- NPCs in Debaucheryville occasionally recognize you

---

### Sinfonia (4 Stamps) — TBD
| Slot | Location | Stamp Design | Signature Beer | Unlock Condition |
|------|----------|--------------|----------------|------------------|
| 1 | TBD | TBD | TBD | TBD |
| 2 | TBD | TBD | TBD | TBD |
| 3 | TBD | TBD | TBD | TBD |
| 4 | TBD | TBD | TBD | TBD |

**Page Theme**: Golden background, musical note borders
**Legendary Hangover**: TBD

---

### Shamsterdam — Expansion (4 + 2 Bonus Stamps) — TBD
| Slot | Location | Stamp Design | Signature Beer | Unlock Condition |
|------|----------|--------------|----------------|------------------|
| 1 | TBD | TBD | TBD | TBD |
| 2 | TBD | TBD | TBD | TBD |
| 3 | TBD | TBD | TBD | TBD |
| 4 | TBD | TBD | TBD | TBD |
| 5 (Bonus) | TBD | TBD | TBD | TBD |
| 6 (Bonus) | TBD | TBD | TBD | TBD |

**Page Theme**: Orange background, tulip/windmill borders
**Legendary Hangover**: TBD
**Expansion Bonus**: 2 additional stamps beyond the standard 4

---

## Continental Beer Master

### Achievement Requirements
- Collect ALL stamps from ALL cities
- Base game: 12 stamps (3 cities × 4 stamps)
- With expansion: 18 stamps (includes Shamsterdam's 6)

### Rewards
- **Title**: "Continental Beer Master" displayed on character
- **Permanent Buff**: +1 to all alcohol-based item effects
- **Discount**: 10% off all alcohol purchases everywhere
- **Secret Ability**: "The Master's Toast" — ultimate toast option
- **Dialogue**: NPCs at all breweries/pubs recognize achievement with special dialogue

### The Master's Toast
- Available only to Continental Beer Masters
- Automatically wins any toast speech challenge
- Grants "Legendary Craic" status to entire party (+5 all stats, 10 min)
- Single use per game day (resets at midnight)

---

## Cross-City Mechanics

### Bottle Opener Collection
Each city's Bottle Opener Keychain is a separate collectible:
- **Publandia Opener**: Required for Publandia breweries
- **Debaucheryville Opener**: Required for Debaucheryville breweries
- **Sinfonia Opener**: Required for Sinfonia breweries
- **Shamsterdam Opener**: Required for Shamsterdam breweries

**"Key Collector" Achievement**: Own all 4 openers (base game) or all 5 (with expansion)

### Legendary Hangover Comparison
| City | Buff | Duration | Penalty | Penalty Duration | Price |
|------|------|----------|---------|------------------|-------|
| Publandia | +15 all stats | Battle | -10 all stats | 30 min | 50 Sovs |
| Debaucheryville | +20 all stats, Confusion immunity | Battle | -15 all, glitch effects | 45 min | 75 Sovs |
| Sinfonia | TBD | TBD | TBD | TBD | TBD |
| Shamsterdam | TBD | TBD | TBD | TBD | TBD |

---

## PNG Asset Requirements (Debaucheryville Additions)

### Stamp Assets (8 PNGs)
- stamp_absinthium_taproom.png - 64x64 - Green fairy in cracked glass
- stamp_tap_dat_vault.png - 64x64 - Soundwave vault door
- stamp_three_crown_keghaus.png - 64x64 - Rusted crowns over stein
- stamp_brew_lab.png - 64x64 - Pixelated hop with blockchain
- stamp_absinthium_animation.png - 64x64x4 - Mist swirl sequence
- stamp_tapdatvault_animation.png - 64x64x4 - Bass pulse sequence
- stamp_keghaus_animation.png - 64x64x4 - Sticky thud sequence
- stamp_brewlab_animation.png - 64x64x4 - Download sequence

### Beer Assets (8 PNGs)
- absinthe_lager_bottle.png - 32x64 - Green glass, glowing
- absinthe_lager_icon.png - 32x32 - Menu icon
- vape_stout_bottle.png - 32x64 - Vapor cloud design
- vape_stout_icon.png - 32x32 - Menu icon
- iron_gut_tripel_goblet.png - 32x64 - Antique goblet
- iron_gut_tripel_icon.png - 32x32 - Menu icon
- blockchain_bock_bottle.png - 32x64 - Pixelated label
- blockchain_bock_icon.png - 32x32 - Menu icon with price fluctuation

### Bottle Opener Assets (2 PNGs)
- bottle_opener_debaucheryville.png - 48x48 - Neon fairy design
- bottle_opener_debaucheryville_glow.png - 48x48 - UV glow variant

### Page Assets (2 PNGs)
- passport_page_debaucheryville.png - 256x384 - Neon purple page
- legendary_hangover_debaucheryville.png - 48x48 - Neon Nightmare icon

---

*The Brewery Passport Book: More than a collectible. More than a tracker. Publandia taught you to drink. Debaucheryville taught you to question reality while drinking. Fill the book. Earn the hangovers. Become the Continental Beer Master.*
