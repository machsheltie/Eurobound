...
| C19 | **Raunch ceiling** | Darkweb ICQ logs (fentanyl gummies, organ trade) far edgier than the rest | **VOID — no ceiling. MA / adults-only, full-strength satire** (author binding rule §0a-2: sanitizing, softening, or de-subversifying is a highest-severity defect). No `SATIRE_MODE=SAFE`, no `line_safe` strings, no safe build — sanitization mechanisms are banned outright. Pre-sanitization vice content stands at full strength unless the author rules otherwise. | The 16+ ceiling and Satire Swap system were an unauthorized sanitization pass. The satire ships intact or not at all; Steam targeting removes the Apple-rating rationale. |
