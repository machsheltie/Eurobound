...
**The rendering rule (binding):** in each of the three passes the camera drops into that bro's first-person POV and Renata's sprite is replaced by `renata_goggles_render` — a generic, anonymous, over-airbrushed 22-year-old composite, flat-shaded and lit slightly wrong for the scene. **It is the identical asset all three times. The sameness is the joke.** It is not a younger Renata; it is not modeled on her at all; it is the bros' own stock daydream dropped over whoever is in front of them. It renders **only** inside these three POV passes and **never** in a neutral camera. *(Full sprite rules: `Design/Character Profiles/DebaucheryvilleNPCs/Bohemian Riviera/queen_riviera_renata.md`, Sprite States.)*
...
- `renata_goggles_render` ships as **one asset used three times.** Do not produce variants, do not age it per-bro, do not "improve" it in a polish pass. It is a generic, anonymous, over-airbrushed composite — **explicitly not a younger Renata and not modeled on her.** It is the bros' own stock daydream, and the sameness across all three POV passes is the entire gag.
- It renders **only in first-person bro POV**, never in a neutral camera, and **drops permanently at the morning cut.** It never renders again anywhere in the game.
- Light it *slightly wrong for the scene* — flat-shaded against the strip's amber string-light bounce — so an attentive player clocks it as an overlay before the morning reveal ever happens.
